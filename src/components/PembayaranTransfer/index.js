...
const PembayaranTransfer = (props) => {
    const {pembayaran, setPembayaran} = props;
    // const [pembayaran, setPembayaran] = useState('null')

    function classTransfer(name) {
        if(pembayaran === name){
            return <div className="space-icon d-flex">
                <div><p>{name}</p></div>
                <div className="space-icon-image"><img src={IconCheckList}></img></div >
            </div>
        } else {
            return <span className="no-space-icon">
                <span>{name}</span>
            </span>
        }
    }
    ...
    return <div className="pembayaran-transfer-section">
        <div>
...
export default PembayaranTransfer;